 ...
 * @Createdate Sat, 26 Mar 2016 00:19:55 GMT
 */
...
$_sql = 'SELECT id,name FROM nv4_vi_textlink_customer';
...
if ( ! $nv_Request->isset_request( 'id', 'post,get' ) )
{
	...
	if( ! empty( $q ) )
	{
		$db->where( 'a_title LIKE :q_a_title OR begin_time LIKE :q_begin_time OR end_time LIKE :q_end_time' );
	}
	$sth = $db->prepare( $db->sql() );

	if( ! empty( $q ) )
	{
		$sth->bindValue( ':q_a_title', '%' . $q . '%' );
		$sth->bindValue( ':q_begin_time', '%' . $q . '%' );
...
if( $show_view )
{
	...
	while( $view = $sth->fetch() )
	{
		...
		$view['customer_id'] = $array_customer_id_textlink[$view['customer_id']]['name'];
